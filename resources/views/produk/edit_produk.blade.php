<x-layout>
    ...
    <form action="{{ route('customer.update',$produk->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 mt-8">
            ...
            <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <input type="text" value="{{ $produk->name_product }}" name="name_product" id="name_product" autocomplete="given-name" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                    </div>
                </div>
            </div>
            <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="mt-2">
                        <select name="category_id" id="category_id" class="...">
                            @foreach ($category as $k)
                                <option value="{{ $k->id }}" {{ $produk->category_id == $k->id ? 'selected' : '' }}>
                                    {{ $k->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="sm:col-span-3 ml-35 -mt-8.5">
            ...
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <input type="text" value="{{ $produk->buy_price }}" name="buy_price" id="buy_price" autocomplete="given-name" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                    </div>
                </div>
            </div>
            ...
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <input type="text" value="{{ $produk->sell_price }}" name="sell_price" id="sell_price" autocomplete="given-name" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-full mt-5">
                ...
                <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                   <input type="file" name="photo" id="photo" value="{{ $produk->photo }}">
                </div>
            </div>
